...
class ProductRepositoryImpl {
  ...
  Future<List<ProductModel>> getFavProducts() async {
    try {
      QuerySnapshot querySnapshot =
          await _firestore.collection('products').get();
      List<ProductModel> favProducts = querySnapshot.docs
          .map((doc) {
            Map<String, dynamic> data = doc.data() as Map<String, dynamic>;
            return ProductModel.fromJson(doc.id, data);
          })
  ...
  Future<List<ProductModel>> getOrderProducts() async {
    try {
      QuerySnapshot querySnapshot =
          await _firestore.collection('products').get();
      List<ProductModel> ordProducts = querySnapshot.docs
          .map((doc) {
            Map<String, dynamic> data = doc.data() as Map<String, dynamic>;
            return ProductModel.fromJson(doc.id, data);
          })
          .toList()
          .where((element) => element.ordProducts)
          .toList();
      return ordProducts;
    } catch (e) {
      print('Error getting products: $e');
